...
function DahboordDoctor({ children }) {
...
const navigate = useNavigate()
  ...
  return (
    <>
      <div className="flex justify-center items-center ">
        <nav className="z-20 flex shrink-0 grow-0 justify-around gap-4 border-t border-gray-200 bg-white/50 p-2.5 shadow-lg backdrop-blur-lg dark:border-slate-600/60 dark:bg-slate-800/50 fixed top-2/4 -translate-y-2/4 left-6 min-h-[auto] min-w-[64px] flex-col rounded-lg border">
          ...
          >
            ...
            <small class="text-center text-xs font-medium"> Logout </small>
          </a>
        </nav>
      </div>
      ...
      {isRender ? (
        <>
          {/* Render children if isRender is true */}
          <div className="flex justify-center items-center h-screen bg-gradient-to-r from-pink-300 via-purple-300 to-indigo-400">
            <div>{children}</div>
          </div>
        </>
      ) : (
        // Render LoadingPage component if isRender is false
        <LodingPage />
      )}
    </>
  );
}
export default DahboordDoctor;
